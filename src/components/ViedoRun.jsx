    ...
    return (
        <div className='flex items-center justify-center max-sm:mt-5'>
            <div className="flex md:w-[90%] gap-4  md:p-4 max-sm:flex-col-reverse max-sm:w-full">
                {/* Related Videos List */}
                <div className="w-[40%] max-sm:w-full overflow-y-auto  overflow-x-hidden  rounded-lg">
        ...
        <p className="text-gray-700 mb-4">
            ...
            {videoDetails.description && videoDetails.description.length > 200 && (
                <button onClick={toggleDescription} className="text-blue-500 ml-2 underline">
                    ...
                    <div className="space-y-2 overflow-y-auto overflow-x-hidden h-screen max-sm:h-[70vh]">
                        {relatedVideos.map((video) => (
                            <div key={video.id.videoId} className="group cursor-pointer" onClick={() => handleVideoSelect(video.id.videoId)}>
                                <div className="flex items-start rounded-md shadow-lg p-2  transition-transform duration-300 transform group-hover:scale-105">
                                    {/* Image Section */}
                                    <img 
                                    ...
                                    <div className="ml-3 flex flex-col justify-between">
                                        <h5 className="text-lg font-semibold text-gray-800">{video.snippet.title}</h5>
